...
% Input: voltage over battery; Output: battery current 
  setup(block)
...
function Update(block)
  ...
  i_c   = Cg*(uk - block.Dwork(3).data)/Delta;
  
  % Calculate prediction for state and cov-Matrix
  Ak        = [[eye(5), zeros(5,2)]; Delta*[xk(7)-xk(2)*xk(6), -xk(1)*xk(6),...
      0, 0, 0, 1/Delta-xk(1)*xk(2), xk(1); -xk(3)*(xk(7)-xk(2)*xk(6)),...
      xk(1)*xk(3)*xk(6), i_c/Cg-xk(7)*(xk(1)+(a(2)+2*a(1)*xk(5))/Qe)+xk(1)*xk(2)*xk(6),...
      0, -2*a(1)*xk(3)*xk(7)/Qe, xk(1)*xk(2)*xk(3), 1/Delta - xk(3)*...
      (xk(1)+(a(2)+2*a(1)*xk(5))/Qe)]];
  Ak(5,7)   = Delta/Qe;
  x_p_kp1   = [xk(1); xk(2); xk(3); xk(4)+Delta*i_c; xk(5)+Delta*xk(7)/Qe; 
      xk(6)+Delta*xk(1)*(xk(7)-xk(2)*xk(6)); xk(7)+Delta*xk(3)*(i_c/Cg-xk(7)*...
      (xk(1)+(a(2)+2*a(1)*xk(5))/Qe+xk(1)*xk(2)*xk(6)))];
