...
% Register number of dialog parameters
  block.NumDialogPrms = 7; %Delta, x0, P0, a, 
                        %Z_k, W_k, Qe
...
function Output(block)
  ...
  y_hat = u_k/x_p_k(3) + u_oc + x_p_k(5); % ib*R0 + u_oc + u_rc
  ...
  % Calculate "simulated" output for further comparison
  u_oc  = a(1)*x_k(4)^2 + a(2)*x_k(4) + a(3);
  y_hat = u_k/x_k(3) + u_oc + x_k(5);
...
function Update(block)
  ...
  Ak        = [[eye(4), zeros(4,1)]; Delta*[uk-xk(2)*xk(5), -xk(1)*xk(5),...
      0, 0, 1/Delta - xk(1)*xk(2)]];
  x_p_kp1   = [xk(1); xk(2); xk(3); xk(4)+Delta*uk/Qe; xk(5)+Delta*xk(1)*...
      (uk-xk(2)*xk(5))];
