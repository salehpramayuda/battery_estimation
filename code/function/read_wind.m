function data = read_wind(filename)
    ...
    % struct.Vector : speed*[north-south, east-west]
    data = repmat(struct('Date','','Time','','Vector',zeros(1,2)),length(data_)-miss,1);
    for i = 1:length(data_)
        ...
        if(spid~=0)
            ...
            data(i).Vector = spid*[round(sin(phase),10),round(cos(phase),10)];
        end
    end
end